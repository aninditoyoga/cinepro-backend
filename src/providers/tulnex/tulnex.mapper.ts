...
export function extractUrl(data: any): ExtractedStream | null {
    ...
    const wrap = (url: unknown, headers: Record<string, string> | null = null): ExtractedStream | null => {
    ...
    const srcsPrimary = (d.sources as Record<string, unknown>)?.primary as Record<string, unknown> | undefined;
    ...
    if (Array.isArray(d.languages)) {
        const orig = (d.languages as Record<string, unknown>[]).find(l => l.original === true && Array.isArray(l.sources) && (l.sources as unknown[]).length > 0);
        if (orig) {
            const sorted = [...(orig.sources as Record<string, unknown>[])].sort((a, b) =>
    ...
    if (Array.isArray(d.links) && d.links.length > 0) {
        const link = (d.links as Record<string, unknown>[]).find(l => typeof l.url === 'string' && (l.url as string).includes('http'));
        if (link) return wrap(link.url, headers);
    }

    const nestedData = d.data as Record<string, unknown> | undefined;
    if (nestedData?.data && (nestedData.data as Record<string, unknown>)?.stream)
        return wrap(((nestedData.data as Record<string, unknown>).stream as Record<string, unknown>)?.playlist, headers);
    if (nestedData?.stream)
        return wrap((nestedData.stream as Record<string, unknown>)?.playlist, headers);
    if (typeof nestedData?.url === 'string' && nestedData.url.includes('http'))
        return wrap(nestedData.url, (nestedData.headers as Record<string, string>) ?? headers);

    if (Array.isArray(nestedData?.sources)) {
        const src = (nestedData!.sources as Record<string, unknown>[]).find(s => typeof s.url === 'string' && (s.url as string).includes('http'));
        if (src) return wrap(src.url, (src.headers as Record<string, string>) ?? headers);
    }

    if (Array.isArray(d.streams)) {
        const src = (d.streams as Record<string, unknown>[]).find(s =>
